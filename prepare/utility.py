...
def get_augmented_cube(suid: str, img: np.array, radii: list, centers: list, main_nodule_idx: int, spacing: tuple,
                       lungs_bounding_box: list, block_size=128, pad_value=106, margin=10, rot_id=None):
    
    print('Preprocessed image size', img.shape)
    print('Series uid', suid)
   # img_file = sitk.GetImageFromArray(img)
   
   # fig = plt.figure()
   # ax = fig.add_subplot(111, projection='3d')
   # ax.plot(img[:, :,0],img[:, :,1],img[:, :,2])
   # plt.plot(img[1,:,:]) 
    ...
    print('Scaled image with scale factor', scale_factor)
    print('Scaled image shape', img1.shape)
    ...
    print('Random cropped image shape', img2.shape)
    # ax.plot(img2[:,0],img2[:,1],img2[:,2])
    # plt.show()
    # plt.title('Random cropped image')
    # plt.imshow(img2[0,:,:], cmap=plt.cm.bone) 
    ...
    print('Rotated by degrees', rotate_id)
    print('Rotated image shape', img3.shape)
    # ax.plot(img3[:,0],img3[:,1],img3[:,2])
    # plt.show()
    # plt.title('Rotated image')
    # plt.imshow(img3[0,:,:], cmap=plt.cm.bone) 
    ...
    # Preprocessed
    plt.subplot(221)
    plt.imshow(img[nth_slice, :, :], cmap=plt.cm.bone)
    plt.title('Original preprocessed image')
    

    # Scaled
    plt.subplot(222)
    plt.imshow(img1[int(nth_slice*scale_factor),:,:], cmap=plt.cm.bone) 
    plt.title('Scaled image')
   

    # Random cropped
    rand_slice_nodule_cube = 34
    plt.subplot(223)
    plt.imshow(img2[rand_slice_nodule_cube,:,:], cmap=plt.cm.bone)
    plt.title('Random cropped of nodule')
  

    # Rotated
    plt.subplot(224)
    plt.imshow(img3[rand_slice_nodule_cube,:,:], cmap=plt.cm.bone)
    plt.title('Rotated image')
   ...
   # plt.show()
    #fig.savefig(f'{OUTPUT_PATH}/Visualise/{random.randint(1, 100)}.png')
    fig.savefig(f'{OUTPUT_PATH}/Visualise/{suid}.png')
