...
class CTScan(object):
    def __init__(self, seriesuid, centers, radii, clazz):
        ...
        print('Series UID', self._seriesuid)
...
class PatchMaker(object):
    ...
    def _get_augmented_patch(self, idx, rot_id=None):
        return get_augmented_cube(suid = self._seriesuid, img=self._image, radii=self._radii, centers=self._coords,
    ...
    def get_augmented_patches(self):
        ...
        for i in range(len(self._coords)):
            ...
            if radii[i] > 15.:
                times_to_sample = 2
                print('Radius > 15', radii[i])
            elif radii[i] > 20.:
                times_to_sample = 6
                print('Radius > 20', radii[i])
            for j in range(times_to_sample):
                rot_id = int((j / times_to_sample) * 24 + np.random.randint(0, int(24 / times_to_sample)))
                print('Rotation by', rot_id)
                print('Times to sample', times_to_sample, 'for series uid', self._seriesuid)
        ...
        print('No of augmented patches for a single series uid', len(self._coords)*times_to_sample)
